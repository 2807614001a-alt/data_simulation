...
def _snapshot_from_chain(events: List[Dict]) -> Dict:
    """从内存中的 action_event_chain 列表构建 snapshot，避免再次读盘。设备开/关由事件层的 device_patches（提示词要求用毕关设备）体现，此处仅汇总链上状态。"""
    ...
    for evt in events:
        ...
        for patch in layer.get("patch_on_start", []) + layer.get("patch_on_end", []):
            ...
            if isinstance(patch_data, list):
                patch_data = {p.get("key"): p.get("value") for p in patch_data if isinstance(p, dict) and p.get("key")}
...
def _load_action_event_chain_snapshot() -> Dict:
    ...
    return _snapshot_from_chain(events)
...
def _init_agent_state(previous_day_summary: str, previous_day_snapshot: Dict) -> Dict[str, object]:
    ...
    summary = previous_day_summary or ""
    # 与 previous_day_summary 对齐：若前日正常则 energy/health 应一致，避免矛盾
    if "睡眠充足" in summary or "能量维持" in summary or "中等偏上" in summary or "未熬夜" in summary:
        if "生病" not in summary and "感冒" not in summary and "头晕" not in summary and "跌倒" not in summary:
            energy = max(0.5, min(0.9, energy + 0.2))
            health = "ok"
            mood = "neutral"
    if "熬夜" in summary or "睡眠不足" in summary:
...
def run_multi_day_simulation() -> None:
    ...
    # 一次性加载 profile / layout / details，14 天循环内复用，减少本地重复读盘
    settings_dir = project_root / "settings"
    profile_data: Dict = {}
    profile_json = "{}"
    layout_data: Dict = {}
    house_details_map: Dict = {}
    planning_context: Optional[Dict] = None
    event_settings: Optional[Dict] = None
    device_settings: Optional[Dict] = None
    if (settings_dir / "profile.json").exists():
        with open(settings_dir / "profile.json", "r", encoding="utf-8") as f:
            profile_data = json.load(f)
        profile_json = json.dumps(profile_data, ensure_ascii=False, indent=2)
    if (settings_dir / "house_layout.json").exists():
        with open(settings_dir / "house_layout.json", "r", encoding="utf-8") as f:
            layout_data = json.load(f)
    if (settings_dir / "house_details.json").exists():
        with open(settings_dir / "house_details.json", "r", encoding="utf-8") as f:
            details_list = json.load(f)
        for item in details_list:
            uid = item.get("furniture_id") or item.get("device_id")
            if uid:
                house_details_map[uid] = item
    if profile_data and layout_data:
        planning_context = planning.build_settings_data_from_cache(profile_data, layout_data)
    if profile_json and layout_data is not None:
        event_settings = {
            "profile_json": profile_json,
            "house_layout": layout_data,
            "house_details_map": house_details_map,
        }
    if house_details_map is not None:
        device_settings = {"house_details_map": house_details_map}
    ...
    # 上一日 event 层结束时的房间环境与设备状态，供 Day2+ 作为当日初值（保证多日环境/设备连续）
    ...
    for day_index in range(1, DAYS + 1):
        ...
        # 室外温湿度：必须使用日变化格式，使自然衰减生效（室内随时刻与室外差异变化）
        outdoor = fetch_openweather()
        prefs = profile_data.get("preferences") or {}
        ow = prefs.get("outdoor_weather")
        if isinstance(ow, dict) and "temperature_min" in ow and "temperature_max" in ow:
            simulation_context["outdoor_weather"] = {
                "temperature_min": float(ow.get("temperature_min", 18)),
                "temperature_max": float(ow.get("temperature_max", 28)),
                "humidity_min": float(ow.get("humidity_min", 0.4)),
                "humidity_max": float(ow.get("humidity_max", 0.75)),
            }
        elif outdoor and isinstance(outdoor.get("temperature"), (int, float)):
            # API 返回单一值时转为日变化，避免 T_outdoor == T_room 导致自然衰减恒为 0
            t = float(outdoor["temperature"])
            h = float(outdoor.get("humidity", 0.5))
            simulation_context["outdoor_weather"] = {
                "temperature_min": max(5, t - 6),
                "temperature_max": min(40, t + 6),
                "humidity_min": max(0.2, h - 0.2),
                "humidity_max": min(0.9, h + 0.2),
            }
        else:
            pref_temp = prefs.get("home_temperature", 24)
            if not isinstance(pref_temp, (int, float)):
                pref_temp = 24
            simulation_context["outdoor_weather"] = {
                "temperature_min": max(5, pref_temp - 6),
                "temperature_max": min(40, pref_temp + 6),
                "humidity_min": 0.4,
                "humidity_max": 0.75,
            }
        ...
        activity_plan = planning.run_planning(
            simulation_context=simulation_context,
            cached_settings_data=planning_context,
        )
        ...
        if RUN_EVENTS:
            print("  Event layer: LLM calls (10-60s per activity, please wait)...", flush=True)
            event_result = event.run_batch_processing(
                activity_plan.get("activities", []),
            ...
            if isinstance(event_result, dict):
                previous_day_env_snapshot = event_result.get("final_environment_snapshot")
                previous_day_device_states = event_result.get("final_device_states")

            print("  Device chain: generating device states...", flush=True)
            action_chain = device_operate.run_event_chain_generation(cached_settings=device_settings)
            _copy_json(
        ...
        if RUN_EVENTS:
            previous_day_snapshot = (
                _snapshot_from_chain(action_chain)
                if action_chain is not None
                else _load_action_event_chain_snapshot()
            )
        else:
