...
# 日志里的 [LLM 耗时] = 单次 HTTP 从发起到收到完整响应的总时间，绝大部分是第三方（base_url）的响应时间；可用根目录 test_api_latency.py 单独测一次确认
...
# 单次 LLM HTTP 请求超时（秒），超时后抛错并触发重试，不无限等待；OPENAI_REQUEST_TIMEOUT 覆盖，默认 120
LLM_REQUEST_TIMEOUT = max(30, _env_int("OPENAI_REQUEST_TIMEOUT", 120))

# 超时/网络错误时对同一活动的重试次数，用尽后才跳过该活动；SIM_LLM_RETRY_COUNT 覆盖，默认 3
LLM_RETRY_COUNT = max(0, _env_int("SIM_LLM_RETRY_COUNT", 3))
# 重试前等待秒数，避免连续打满 API；SIM_LLM_RETRY_DELAY 覆盖，默认 10
LLM_RETRY_DELAY = max(1, _env_int("SIM_LLM_RETRY_DELAY", 10))

# 单次 LLM 调用（如 generate/validate/correct）遇到连接/5xx 时的内层重试次数与间隔；避免一次网络抖动就消耗整次「活动级」重试
INNER_LLM_RETRY_COUNT = max(0, _env_int("SIM_INNER_LLM_RETRY_COUNT", 3))
INNER_LLM_RETRY_DELAY = max(1, _env_int("SIM_INNER_LLM_RETRY_DELAY", 2))
...
# 模拟天数，SIM_DAYS 覆盖
DAYS = max(1, _env_int("SIM_DAYS", 7))
...
# 城市名（英文，如 Beijing, Shanghai），用于拉取室外温湿度；OPENWEATHER_CITY / WEATHER_CITY 覆盖
OPENWEATHER_CITY = _env("OPENWEATHER_CITY", "Beijing").strip() or _env("WEATHER_CITY", "Beijing").strip() or "Beijing"
...
MAX_PLANNING_REVISIONS = max(0, _env_int("SIM_MAX_PLANNING_REVISIONS", 3))
...
# 是否按段生成事件并每段后调用物理引擎推进环境（默认开启），使环境变化与设备操作形成因果链
USE_ITERATIVE_EVENT_GENERATION = _env_bool("USE_ITERATIVE_EVENT_GENERATION", True)

# 校验未过时最多修正几轮，SIM_MAX_EVENT_REVISIONS 覆盖
MAX_EVENT_REVISIONS = max(0, _env_int("SIM_MAX_EVENT_REVISIONS", 3))
